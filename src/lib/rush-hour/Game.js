  ...
  for (const [sym, meta] of this.pieceMap.entries()) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (r < 0 || r >= this.rows || c < 0 || c >= this.cols) {
        console.error(`Out of bounds placing '${sym}':`, { r, c, thisRows: this.rows, thisCols: this.cols });
      }
      grid[r][c] = sym;
    }
  }
...
Game.prototype._isEmpty = function (grid, r, c) {
  return (
    r >= 0 && r < this.rows && c >= 0 && c < this.cols && grid[r][c] === null
  );
};
...
Game.prototype.aStarSearch = function () {
  ...
  const push = (n) => {
    ...
    frontier.sort(
      (a, b) =>
        (a.cost + this._heuristic(a.state)) -
